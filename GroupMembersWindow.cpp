  ...
  void handle_input(windows::PadWindow &root, TickitKeyEventInfo *info) override {
    if (info->type == TICKIT_KEYEV_KEY) {
      if (!strcmp(info->str, "Enter")) {
        if (chat_) {
          static_cast<MenuWindow &>(root).spawn_submenu(ChatInfoWindow::spawn_function(chat_));
        } else {
          static_cast<MenuWindow &>(root).spawn_submenu(ChatInfoWindow::spawn_function(user_));
        }
      }
    }
  }
